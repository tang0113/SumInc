...
def main():
    ...
    # cmd: python3 gen_inc.py /mnt/data/nfs/yusong/dataset/large/google/google.e 0.01 -w=0
    ...
    if not os.path.exists('%s/%.3f' % (prefix, percentage)):
        os.mkdir('%s/%.3f' % (prefix, percentage))
    if have_weight:
        base += '_w'
    fp_vm = open('%s/%s_%.3f.v' % (prefix, base, percentage), 'w')
    fp_base = open('%s/%.3f/%s.base' % (prefix, percentage, base), 'w')
    fp_append = open('%s/%.3f/%s.update' % (prefix, percentage, base), 'w')
    fp_updated = open('%s/%.3f/%s.updated' % (prefix, percentage, base), 'w')

    print('base edge:', '%s/%.3f/%s.base' % (prefix, percentage, base))
