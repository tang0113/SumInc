...
for name in europe_osm_small_1b #webbase-2001 uk-2005 uk-2002 arabic-2005 it-2004 gsh-2015-tpd sk-2005
do
    # tar -C ${dataset_path}/${name} -xzvf ${dataset_path}/${name}.tar.gz
    for rate in 0.0001
    do
    {
        for weight in 0 1 #1 # 权
        do
        {
            cmd="python3 ${python_path}/gen_inc2_old.py ${dataset_path}/${name}/${name}_ud.e ${rate} -w=${weight}"
            echo $cmd
            eval $cmd
            echo -e "\n"
        } #&
...
# do
#     for rate in 0.001 0.01 #0.01 0.001 0.0001
#     do
#     {
#         for weight in 1 0 # 1 # weight
#         do
#         {
#             cmd="python3 ${python_path}/gen_inc2_old.py ${dataset_path}/${name}/${name}.mtx ${rate} -w=${weight} -header"
#             echo $cmd
#             eval $cmd
#         } &
#         done
