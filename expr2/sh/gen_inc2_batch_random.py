...
def main():
    if len(sys.argv) < 3:
        sys.exit("Usage: python gen.py [path] [changesize] [-w=0/1] [-header]") # -w=0无权， -w=1有权
    
    # cmd: python3 gen_inc2_batch_random.py /mnt2/neu/yusong/dataset/large/google/google_w_ud.e 1000 -w=1

    path = sys.argv[1]
    change_size = int(sys.argv[2])
    header = False
    have_weight = False
    split_ = ' '
    if len(sys.argv) >= 4:
    ...
    # add_size = int(num_lines * change_size / 2)
    # del_size = int(num_lines * change_size / 2)
    add_size = change_size / 2
    del_size = change_size / 2

    print("Vertex num: ", vertex_num)
    print("Add size: ", add_size)
    print("Del size: ", del_size)

    if not os.path.exists('%s/%d' % (prefix, change_size)):
        os.mkdir('%s/%d' % (prefix, change_size))
    if have_weight:
        base += '_w'
    fp_vm = open('%s/%s_%d.v' % (prefix, base, change_size), 'w')
    fp_base = open('%s/%d/%s.random.base' % (prefix, change_size, base), 'w')
    fp_append = open('%s/%d/%s.random.update' % (prefix, change_size, base), 'w')
    fp_updated = open('%s/%d/%s.random.updated' % (prefix, change_size, base), 'w')

    print('base edge:', '%s/%d/%s.base' % (prefix, change_size, base))
    ...
    processed_edges = set()
    while add_size > 0:
        u_gid = random.randint(0, vertex_num-1)
        oes = G[u_gid]
        if len(oes) > 1:
