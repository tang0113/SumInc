...
def main():
    ...
    for u_gid in G:
        oes = G[u_gid]
        old_size = len(oes)
        reserve_num = math.ceil(old_size * 0.2)
        # while len(oes) > 1 and add_size > 0:
        while len(oes) > reserve_num and add_size > 0:
    ...
    for u_gid in G:
        ...
        old_size = len(oes)
        reserve_num = math.ceil(old_size * 0.2)
        reserve_num = max(reserve_num, 2)
        # while len(oes) > 2 and del_size > 0:
        while len(oes) > reserve_num and del_size > 0:
            v_gid, weight = pop_first(oes)
            if have_weight:
