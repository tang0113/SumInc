...
app_root_path=/mnt2/neu/yusong/code/SumInc/expr2/utils
...
do
  for name in webbase-2001 #uk-2005 europe_osm road_usa arabic-2005 it-2004 gsh-2015-tpd sk-2005 webbase-2001 # uk-2002 
  do
      cmd="g++ ${app_root_path}/deal_dataset.cc -lgflags -lglog -o ${app_root_path}/deal_dataset && ${app_root_path}/deal_dataset -weight=0 -base_edge=${root_path}/dataset/expr_data/${name}/${name}${fix} -out_edge=${root_path}/dataset/expr_data_${rand}/${name}/${inc_rate}/${name}${fix} -out_dir=/mnt2/neu/yusong/dataset/expr_data_${rand}/${name}/ -inc_rate=${inc_rate} -hava_inadj=1 -head=0" # 必须手动配置head
      echo $cmd
      eval $cmd
