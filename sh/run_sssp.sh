...
do
    ...
    # ingress
    cmd="mpirun -n 1 ${app_root_path}/build/ingress -application ${app} ${vfile} ${efile} ${update} -directed=1 -cilk=true -termcheck_threshold ${termcheck_threshold} -app_concurrency ${app_concurrency} -compress=0 -portion=1 -min_node_num=5 -max_node_num=${max_node_num} -sssp_source=${source} -php_source=${source} -compress_concurrency=1 -build_index_concurrency=${build_index_concurrency} -compress_type=2 ${serialization_prefix} -compress_threshold=${compress_threshold} -message_type=${type} -verify=0 ${ingress_out}" # -out_prefix ${out_prefix}/Ingress_${app}
    # echo $cmd
    # eval $cmd
    # echo $ingress_out
