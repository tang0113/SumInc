 ...
 public:
  ...
  void deltaCompute() {
    ...
    // debug
    {
      ...
      LOG(INFO) << "  delta_sum=" << delta_sum;
      LOG(INFO) << "  value_sum=" << value_sum;
        LOG(INFO) << "  test------------------351458.v="
                  << app_->values_[vertex_t(351458)]
                  << " is=" << int(app_->active_entry_node_[vertex_t(351458)]);
        LOG(INFO) << "  test------------------674055.v="
                  << app_->values_[vertex_t(674055)]
                  << " is=" << int(app_->active_entry_node_[vertex_t(674055)]);
    }
    ...
    // debug
    {
      ...
      LOG(INFO) << "  value_sum=" << value_sum;
          ...
          LOG(INFO) << "  test------------------351458.v="
                    << app_->values_[vertex_t(351458)]
                    << " is=" << int(app_->active_entry_node_[vertex_t(351458)])
                    << " active=" << app_->curr_modified_.Exist(vertex_t(351458));
          LOG(INFO) << "  test------------------674055.v="
                    << app_->values_[vertex_t(674055)]
                    << " is=" << int(app_->active_entry_node_[vertex_t(674055)])
                    << " active=" << app_->curr_modified_.Exist(vertex_t(674055));
          LOG(INFO) << "  test------------------513753.v="
                    << app_->values_[vertex_t(513753)]
                    << " is=" << int(app_->active_entry_node_[vertex_t(513753)])
                    << " active=" << app_->curr_modified_.Exist(vertex_t(513753));
          LOG(INFO) << "  test------------------513754.v="
                    << app_->values_[vertex_t(513754)]
                    << " is=" << int(app_->active_entry_node_[vertex_t(513754)])
                    << " active=" << app_->curr_modified_.Exist(vertex_t(513754));
          LOG(INFO) << "  test------------------598405.v="
                    << app_->values_[vertex_t(598405)]
                    << " is=" << int(app_->active_entry_node_[vertex_t(598405)])
                    << " active=" << app_->curr_modified_.Exist(vertex_t(598405));
    }
  }
  ...
  void first_step(VertexArray<value_t, vid_t>& values_temp,
    ...
    app_->active_entry_node_.Swap(active_entry_node_temp);
    parallel_for (vid_t i = 0; i < node_range[4]; i++) {
      ...
      app_->active_entry_node_[v] = active_entry_node_temp[vertex_t(newId2oldId[i])];
    ...
    {
      LOG(INFO) << "  test-++++--------------351458.v="
              << app_->values_[vertex_t(oldId2newId[351458])]
              << " is=" 
              << int(app_->active_entry_node_[vertex_t(oldId2newId[351458])]);
      LOG(INFO) << "  test-++++-----------------674055.v="
              << app_->values_[vertex_t(oldId2newId[674055])]
              << " is=" 
              << int(app_->active_entry_node_[vertex_t(oldId2newId[674055])]);
    }
      /* send one round */
      ForEachCilkOfBitset(
        app_->curr_modified_, 
  ...
  void Query() {
    ...
    active_entry_node_temp.Init(fragment_->InnerVertices(), 0);
    fid_t fid = fragment_->fid();
    auto vm_ptr = fragment_->vm_ptr();
...
// #define DEBUG
      ...
      if (terminate) {
        if(compr_stage){
          ...
          parallel_for (vid_t i = node_range[0]; i < node_range[4]; i++){
            ...
            active_entry_node_temp[old_v] = app_->active_entry_node_[v];
          ...
          // print_result("校正前:");
          std::vector<char> active_cluster;
          active_cluster.resize(cpr_->GetClusterSize(), 0);
          parallel_for (vid_t i = 0; i < cpr_->old_node_num; i++) {
            vertex_t v(i);
            if (app_->active_entry_node_[v]) {
              active_cluster[cpr_->id2spids[v]] = 1;
            }
          }
          ...
          active_cluster[cpr_->id2spids[source]] = 1;
          ...
          parallel_for(vid_t j = 0; j < cpr_->supernodes_num; j++){
            ...
            // if (active_cluster[cpr_->id2spids[u]] == 1) {
              auto& value = app_->values_[u];
              if (value != app_->GetIdentityElement() 
                  // && app_->active_entry_node_[u] == 1) { // right, 这是其实不能判断是否是被更新的！老的其实不用发！
                  ) { // right, 这是其实不能判断是否是被更新的！老的其实不用发！
                auto& delta = app_->deltas_[u];
          ...
          parallel_for(vid_t j = 0; j < cpr_->old_node_num; j++){
            app_->active_entry_node_[vertex_t(j)] = 0;
            active_entry_node_temp[vertex_t(j)] = 0;
          }
 ...
 private:
  ...
  VertexArray<char, vid_t> active_entry_node_temp; // for internal value assignment
};
