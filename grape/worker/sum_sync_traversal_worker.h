...
// #define DEBUG
    ...
    if(1){
      ...
      auto &values = app_->values_;
      auto &deltas = app_->deltas_;
      value_t *deltas_d, *deltas_h = (value_t *)malloc(sizeof(value_t) * (FLAGS_compress ? cpr_->all_node_num : num));
      vid_t *deltas_parent_d, *deltas_parent_h = (vid_t *)malloc(sizeof(vid_t) * (FLAGS_compress ? cpr_->all_node_num : num)); 
      value_t *values_d;

      cudaSetDevice(0);
      vid_t *oeoffset_d, *oeoffset_h;
      vid_t *iboffset_d, *iboffset_h;
      vid_t *isoffset_d, *isoffset_h;

      value_t *oe_edata_d, *oe_edata_h;
      value_t *ib_edata_d, *ib_edata_h;
      value_t *is_edata_d, *is_edata_h;
      
      vid_t *is_eparent_d, *is_eparent_h;

      if(!FLAGS_segment){
        oeoffset_h = (vid_t *)malloc(sizeof(vid_t) * oe_offsize);//Ingress,记录每个顶点的邻居，形成一条链表
      ...
      if(FLAGS_segment){
        cudaHostAlloc(&oeoffset_h, sizeof(vid_t) * oe_offsize, cudaHostAllocMapped);
        cudaHostAlloc(&iboffset_h, sizeof(vid_t) * ib_offsize, cudaHostAllocMapped);
        cudaHostAlloc(&isoffset_h, sizeof(vid_t) * is_offsize, cudaHostAllocMapped);

        cudaHostAlloc(&oe_edata_h, sizeof(value_t) * oe_offsize, cudaHostAllocMapped);
        cudaHostAlloc(&ib_edata_h, sizeof(value_t) * ib_offsize, cudaHostAllocMapped);
        cudaHostAlloc(&is_edata_h, sizeof(value_t) * is_offsize, cudaHostAllocMapped);
        cudaHostAlloc(&is_eparent_h, sizeof(vid_t) * is_offsize, cudaHostAllocMapped);
      }
      ...
      // LOG(INFO) << "ib is "<<ib_offsize;
      // LOG(INFO) << "is is "<<is_offsize;
      ...
      if(FLAGS_segment){
        cudaHostGetDevicePointer((void **)&oeoffset_d, (void *)oeoffset_h, 0);
        cudaHostGetDevicePointer((void **)&iboffset_d, (void *)iboffset_h, 0);
        cudaHostGetDevicePointer((void **)&isoffset_d, (void *)isoffset_h, 0);
        
        cudaHostGetDevicePointer((void **)&oe_edata_d, (void *)oe_edata_h, 0);
        cudaHostGetDevicePointer((void **)&ib_edata_d, (void *)ib_edata_h, 0);
        cudaHostGetDevicePointer((void **)&is_edata_d, (void *)is_edata_h, 0);
        cudaHostGetDevicePointer((void **)&is_eparent_d, (void *)is_eparent_h, 0);
      }
        ...
        if ( (terminate && !FLAGS_gpu_start) || (!cur_modified_size_h[0] && FLAGS_gpu_start) ) {//if(app_->next_modified_.Count() == 0)
          ...
          if (batch_stage) {
            ...
            if (!FLAGS_efile_update.empty()) {
              ...
              if(FLAGS_segment){
                cudaFreeHost(oeoffset_h);
                cudaFreeHost(iboffset_h);
                cudaFreeHost(isoffset_h);

                cudaFreeHost(oe_edata_h);
                cudaFreeHost(ib_edata_h);
                cudaFreeHost(is_edata_h);
                cudaFreeHost(is_eparent_h);
              }
    ...
    if(0){//分段的代码
      if(FLAGS_segment){
        ...
        vid_t *cur_seg_d, *seg_num_d;
        vid_t *ib_average_edges_d, *is_average_edges_d;

        vid_t *ib_average_edges_h = (vid_t *)malloc(sizeof(vid_t) * 1);
        vid_t *is_average_edges_h = (vid_t *)malloc(sizeof(vid_t) * 1);
        vid_t *seg_num_h = (vid_t *)malloc(sizeof(vid_t) * 1);
        ...
        cudaMalloc(&cur_seg_d,sizeof(vid_t)*1);
        cudaMalloc(&seg_num_d,sizeof(vid_t)*1);

        cudaMalloc(&ib_average_edges_d,sizeof(vid_t)*1);
        cudaMalloc(&is_average_edges_d,sizeof(vid_t)*1);
        ...
        while(true){
          ...
          if ( (terminate && !FLAGS_gpu_start) || (!cur_modified_size_h[0] && FLAGS_gpu_start) || step > 100 * FLAGS_seg_num ) {
              ...
              LOG(INFO) <<"real time is "<<realtime;
              timetemp = 0;
              realtime = 0;
              if(FLAGS_gpu_start && batch_stage){
                  deltas.fake2buffer();
              ...
              if (!FLAGS_efile_update.empty()) {
                ...
                if (compr_stage == true) {
                  ...
                  //释放内存并且重新分配，size发生变化的变量需要重新分配
                  {
                    ...
                    for(auto v:fragment_->InnerVertices()){
                      if(app_->curr_modified_.Exist(v)){
                        // LOG(INFO)<<"size is "<<app_->curr_modified_.Count();
                        last_modified_h[v.GetValue()] = 1;
                        numz++;
                      }
                    }
                    LOG(INFO) << "size is "<<numz;
                    ib_average_edges_h[0] = ib_average_edges;
                    is_average_edges_h[0] = is_average_edges;
                    seg_num_h[0] = FLAGS_seg_num;
                    cudaMemcpy(ib_seg_start_d, ib_seg_start, sizeof(vid_t) * FLAGS_seg_num, cudaMemcpyHostToDevice);
    ...
    // LOG(INFO) << "id i "<< fragment_->GetId(vertex_t(220377)) <<"id 2"  << fragment_->GetId(vertex_t(220384));
