...
class ParallelEngine {
 public:
   ...
   */
  template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEachCompress(const VertexRange<VID_T>& range,
                      const ITER_FUNC_T& iter_func, int thread_num = 2) {
    std::vector<std::thread> threads(thread_num);
    std::atomic<VID_T> cur(range.begin().GetValue());
    VID_T begin = range.begin().GetValue();
    VID_T end = range.end().GetValue();

    VID_T chunk_size = (end - begin) / thread_num + 1;
  ...
  template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEachIndex(const VID_T range,
                      const ITER_FUNC_T& iter_func, int thread_num = 2) {
    std::vector<std::thread> threads(thread_num);
    VID_T end = range;
    VID_T chunk_size = (end - 0) / thread_num + 1;

    for (uint32_t i = 0; i < thread_num; ++i) {
      threads[i] = std::thread(
          [chunk_size, &iter_func, end](uint32_t tid) {
              VID_T begin = tid * chunk_size;
              iter_func(tid, begin, std::min(begin + chunk_size, end));
          },
          i);
      setThreadAffinity(threads[i], i);
    }
  ...
  /* Simple empty thread */
  template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEach(const VID_T range,
                      const ITER_FUNC_T& iter_func, int thread_num = 2) {
    std::vector<std::thread> threads(thread_num);

    for (uint32_t i = 0; i < thread_num; ++i) {
      threads[i] = std::thread(
          [&iter_func](uint32_t tid) {
              iter_func(tid);
          },
          i);
      setThreadAffinity(threads[i], i);
